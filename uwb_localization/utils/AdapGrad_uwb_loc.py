...
class localization():
    ...
    # AdaGrad algorithm
    def adaptive_gradient_descent(self, prev_location, anchors_location, uwb_distances, weights, learning_rate=1, num_iterations=5000):
        # Initialize the coordinates of the unknown point
        target_location = prev_location
        iteration = 0
        # loss_value = 1000

        gradient_threshold=0.00001 * (np.linalg.norm(weights)/np.linalg.norm(np.ones(len(weights))))  # norm of weights ranging from 0 to > 1, thus the norm needs to be normalized w.r.t max length
        ...
        while iteration < num_iterations:
            ...
            for i in range(3):
                ...
                target_location[i] = target_location[i] - alpha * gradient[i]
    ...
    def AdaGrad (self, prev_location, anchors_location, uwb_distances, weights): # anchor_id starts from 1
        ...
        while n_tries < max_n_tries and loss_value > loss_threshold:
            ...
            loc, loss_value, n_iterations = self.adaptive_gradient_descent(prev_location, anchors_location, uwb_distances, weights)
    ...
    def localization(self, prev_location, all_anchor_data):
        ...
        if len(data_point[:,0]) > 2:
            ...
            curr_location, loss_value, n_iterations = self.AdaGrad(prev_location, anchors_location, uwb_distances, weights)
