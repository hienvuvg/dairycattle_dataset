...
inference_config_file = '/nfs/oprabhune/MmCows/multiple_runs_fast_inference/my_ultralytics_2/inference_config_2.json'
...
print(f"Fold: {fold}")
print(f"Behavior Model Path: {behav_model_path}")
print(f"Standing ID Model Path: {standing_ID_model_path}")
print(f"Lying ID Model Path: {lying_ID_model_path}")
print(f"Save Predictions Directory: {save_preds_directory}")

print("\nNormalization Parameters for Behavior:")
print(f"  Mean: {mean_behav}")
print(f"  Standard Deviation: {std_behav}")

print("\nNormalization Parameters for Standing ID:")
print(f"  Mean: {mean_standing_ID}")
print(f"  Standard Deviation: {std_standing_ID}")

print("\nNormalization Parameters for Lying ID:")
print(f"  Mean: {mean_lying_ID}")
print(f"  Standard Deviation: {std_lying_ID}")
...
def remove_duplicate_bboxes(predictions):
    ...
    for prediction in predictions:
        ...
        if class_label not in selected_bboxes:
            selected_bboxes[class_label] = prediction
        
        elif behavior != 7 and selected_bboxes[class_label][-2] == 7:
            selected_bboxes[class_label] = prediction

        elif behavior != 7 and selected_bboxes[class_label][-2] != 7 and confidence > selected_bboxes[class_label][-1]:
            selected_bboxes[class_label] = prediction
        ...
        print('\nselected_bboxes')
        for s in selected_bboxes:
            print(s)
    # Extract the filtered bounding boxes
    filtered_bboxes = list(selected_bboxes.values())
...
behavior_classifier = create_OMK_model(num_classes = 7).to(device)
behavior_classifier.load_state_dict(torch.load(behav_model_path))
behavior_classifier.eval()

# Create and load lying model
lying_ID_classifier = create_OMK_model(num_classes = 16).to(device)
lying_ID_classifier.load_state_dict(torch.load(lying_ID_model_path))
...
standing_ID_classifier.load_state_dict(torch.load(standing_ID_model_path))
        ...
        for result in results:
            ...
            for index, obj in enumerate(result.boxes):
                ...
                if cropped_obj.size[0] > 0 and cropped_obj.size[1] > 0:  # Ensure cropped image is valid
                    ...
                    print('Index that went in the pipeline', index)
                    crop_copy = copy.deepcopy(cropped_obj)
                    crop_copy.save(f"copy_cropped_im_{index}.jpeg")
                    input_tensor_behav = behav_preprocess(crop_copy).unsqueeze(0).to(device) # Add batch dimension
                    ...
                    predicted_behavior = int(predicted_behavior.item()) + 1 # Add 1 to match the actual labels
                    ...
                    predicted_ID = predicted_ID.item() + 1 # +1 to match ground truth
            ...
            # print('Results : ' , custom_result_list)
